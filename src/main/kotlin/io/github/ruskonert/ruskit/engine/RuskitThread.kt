...
abstract class RuskitThread : SustainableHandler(), Listener, Activator<IntegratedPlugin?>
{
    companion object
    {
        private val named_instance : Set<RuskitThread>  = HashSet()
        fun getActiveRegister() : Set<RuskitThread> = named_instance
    ...
    {
        for (core in RuskitThread.getActiveRegister()) {
            if (core == this)
            {
                return true
            }
    ...
    fun isActivated(): Boolean = RuskitThread.allFramework.any { it == this }
